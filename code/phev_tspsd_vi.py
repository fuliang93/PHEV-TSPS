...
speDis = 0.1 # m/s  # Speed discretization level
...
#     Identify energy consumption
    ...
    spDic = {}
    for (i,j) in X.keys():
        L = [nu for nu in np.arange(vMin,vUbDist[i,j] + 0.01, speDis)] # Speed levels 
        for l in range(len(L)):
            spDic[(i,j,l)] = L[l]        
    Z = m.addVars([ele for ele in spDic.keys()],vtype=GRB.BINARY, name='z') # z_{ijl}
    m.addConstrs(gp.quicksum(Z[ele] for ele in Z.keys() if (ele[0],ele[1]) == (i,j)) == X[i,j] for (i,j) in X.keys()) # 
    vs = m.addVars(arc, name='v') # v_{ij}
    m.addConstrs(vs[i,j] == gp.quicksum(Z[ele] * spDic[ele] for ele in Z.keys() if (ele[0],ele[1])==(i,j)) for (i,j) in arc)
    ...
    # Constraint
    m.addConstrs(Es[i,j] >= arcDist[i,j]/etaD * (mas*g*np.sin(graDist[i,j])*X[i,j] + 0.5*Cd*rho*A*gp.quicksum(Z[ele]*spDic[ele]**2 for ele in spDic.keys() if (ele[0], ele[1]) == (i,j)) + Cr*mas*g*np.cos(graDist[i,j])*X[i,j]) / 10**6  for (i,j) in arc)
    m.addConstrs(Es[i,j] >= arcDist[i,j] * etaG * (mas*g*np.sin(graDist[i,j])*X[i,j] +0.5*Cd*rho*A*gp.quicksum(Z[ele]*spDic[ele]**2 for ele in spDic.keys() if (ele[0], ele[1]) == (i,j)) + Cr*mas*g*np.cos(graDist[i,j])*X[i,j]) / 10**6 for (i,j) in arc)
    ...
    # Travel time budget
    m.addConstr(gp.quicksum(arcDist[i,j]*gp.quicksum(Z[ele]/spDic[ele] for ele in spDic.keys() if (ele[0], ele[1]) == (i,j)) for (i,j) in arc) <= TB ) # (4.45)
...
# =============================================================================
    ...
    # Cumulation cut #
    m.addConstr( gp.quicksum(we[i,j] + mu*wb[i,j] + wr[i,j] for (i,j) in arc) <= ys[0] - ys[n+1] )
    ...
    m.addConstr(Phis[1] <= ys[0] - ys[n+1] - gp.quicksum(wr[i,j] for (i,j) in arc) + (1-deltas[1])*EMaxSum)
    m.addConstr(Phis[1] <= deltas[1]*EMaxSum)
    m.addConstr(Phis[2] >= ys[0] - ys[n+1] - gp.quicksum(wr[i,j] for (i,j) in arc) - (1-deltas[2])*Cbat*n )
    m.addConstr(Phis[2] >= - deltas[2]*EMaxSum )
    m.addConstr(Phis[2]*mu <= ys[0] - ys[n+1] - gp.quicksum(wr[i,j] for (i,j) in arc) + (1-deltas[2])*EMaxSum)
    m.addConstr(Phis[2]*mu <= deltas[2]*EMaxSum)
    m.addConstr(Phis[3]*mu >= ys[0] - ys[n+1] - gp.quicksum(wr[i,j] for (i,j) in arc) - (1-deltas[3])*EMaxSum)
...
# =============================================================================
    ...
    m._we,m._wb,m._wf,m._wr,m._ys = we,wb,wf,wr,ys
...
#    m.Params.CliqueCuts = 2
    ...
    ResDf.to_csv('../result/PHEV' + 'TSPSD_VI_' + numCus + '_' + str(speDis) + '.csv', index=True, sep=',', encoding='utf_8_sig')
